...
async function generate() {
  ...
  for (const post of posts) {
    ...
    const file = await fs.readFile(path.join(__dirname, '..', 'pages', 'blog', post))
  ...
  for (const frontmatter of sortedData) {
    ...
    const stat = statSync(path.join(__dirname, '..', 'public', frontmatter.data.image))
    feed.item({
      title: frontmatter.data.title,
      url: 'https://hr.penci.me/blog/' + frontmatter.slug, // intentionally including slash here
      date: frontmatter.data.date,
      description: frontmatter.data.description,
      enclosure: {
        url: 'https://hr.penci.me' + frontmatter.data.image, // intentionally omitting slash here
        type: 'image/png',
        size: stat.size,
      },
    })
